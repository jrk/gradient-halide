...
DERIVATIVE_TESTS = $(shell ls $(ROOT_DIR)/test/derivative/*.cpp)
...
test_derivative: $(DERIVATIVE_TESTS:$(ROOT_DIR)/test/derivative/%.cpp=derivative_%)
...
run_tests: $(ALL_TESTS)
	make -f $(THIS_MAKEFILE) test_performance test_auto_schedule test_derivative
...
build_tests: $(CORRECTNESS_TESTS:$(ROOT_DIR)/test/correctness/%.cpp=$(BIN_DIR)/correctness_%) \
	...
	$(DERIVATIVE_TESTS:$(ROOT_DIR)/test/derivative/%.cpp=$(BIN_DIR)/derivative_%)
...
derivative_%: $(BIN_DIR)/derivative_%
	@-mkdir -p $(TMP_DIR)
	cd $(TMP_DIR) ; $(CURDIR)/$<
	@-echo
